...
cam_dir = vis_dir + '/dense_imnet_full_base_cam'
...
json_file = open('../models/dense_imnet_full_base_model.json', 'r')
loaded_model = json_file.read()
json_file.close()
model = model_from_json(loaded_model)

model.load_weights('../models/dense_imnet_full_base_weights.hdf5')
model.summary()
...
plot_roc(val_preds, val_slice, 'dense_imnet_full_base')
...
preds_bool = save_preds(preds, test_filenames, test_slice, 'dense_imnet_full_base')
        
pred_idx = utils.find_layer_idx(model, 'predictions')
conv_idx = utils.find_layer_idx(model, 'conv5_block16_concat')
